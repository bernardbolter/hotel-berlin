...
type SpecItem = {
  icon: 'size' | 'bed' | 'occupancy' | 'bathroom'
  label: string
  value: string
}
...
export function RoomSpecStrip({ items, className = '' }: Props) {
  ...
  return (
    ...
    >
      {visible.map((item) => {
        ...
        return (
          <li key={item.icon} className="flex min-w-28 flex-col gap-1">
            ...
            <span className="flex items-center gap-1.5 font-ui text-base font-medium text-hbb-black">
              <Icon size={14} aria-hidden="true" className="text-hbb-rooms-highlight" />
              {item.value}
            </span>
          </li>
        )
